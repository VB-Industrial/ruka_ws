...
int main(int argc, char** argv) {
	...
	robot->getJointModelGroup("ruka_arm_controller")->getVariableDefaultPositions("ready", target_state);
	target_state["base_link__link_01"] = +TAU / 8;

	// define initial scenes
	auto initial_scene{ std::make_shared<planning_scene::PlanningScene>(robot) };
	initial_scene->getCurrentStateNonConst().setToDefaultValues(robot->getJointModelGroup("ruka_arm_controller"), "sleep");
...

	{
		...
		scene->getCurrentStateNonConst().setVariablePositions({ { "base_link__link_01", -TAU / 8 } });
	...
	auto add_to_fallbacks{ [&](auto& solver, auto& name) {
		...
		move_to->setGroup("ruka_arm_controller");
