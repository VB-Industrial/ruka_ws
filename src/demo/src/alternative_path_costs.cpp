...
int main(int argc, char** argv) {
	...
	assert(t.getRobotModel()->getName() == "ruka");

	auto scene{ std::make_shared<planning_scene::PlanningScene>(t.getRobotModel()) };
	auto& robot_state{ scene->getCurrentStateNonConst() };
	robot_state.setToDefaultValues();
	robot_state.setToDefaultValues(robot_state.getJointModelGroup("ruka_arm_controller"), "extended");
	...
	auto alternatives{ std::make_unique<Alternatives>("connect") };
	{
		auto connect{ std::make_unique<stages::Connect>(
			 "path length", stages::Connect::GroupPlannerVector{ { "ruka_arm_controller", pipeline } }) };
	...
	{
		auto connect{ std::make_unique<stages::Connect>(
			 "trajectory duration", stages::Connect::GroupPlannerVector{ { "ruka_arm_controller", pipeline } }) };
		connect->setCostTerm(std::make_unique<cost::TrajectoryDuration>());
		alternatives->add(std::move(connect));
	}
	{
		auto connect{ std::make_unique<stages::Connect>(
			 "eef motion", stages::Connect::GroupPlannerVector{ { "ruka_arm_controller", pipeline } }) };
		connect->setCostTerm(std::make_unique<cost::LinkMotion>("ruka_arm_controller"));
		alternatives->add(std::move(connect));
	}
	{
		auto connect{ std::make_unique<stages::Connect>(
			 "elbow motion", stages::Connect::GroupPlannerVector{ { "ruka_arm_controller", pipeline } }) };
		connect->setCostTerm(std::make_unique<cost::LinkMotion>("link_02__link_03")); // were 4
		alternatives->add(std::move(connect));
	}
	...
	goal_scene->getCurrentStateNonConst().setToDefaultValues(robot_state.getJointModelGroup("ruka_arm_controller"), "start");
